def adding(palk,inimesed):
    add=input("Sisestage  nimi: ")
    inimesed.append(add)
    add_zp=int(input("Sisestage palk : "))
    palk.append(add_zp)
    return palk,inimesed
def adding(palk,inimesed):
    add=input("Isiku nimi:  ")
    inimesed.append(add)
    add_palk=int(input("Inimese palk  " ))
    palk.append(add_palk)
    return palk,inimesed
    print()
    print()
def delete(palk, inimesed):
    x=input("Kas kasutajanimi on kirjutatud tähtedega – 1 või numbritega – 2? ")
    if x=="2":
        i=int(input("Sisestage number: "))
        palk.pop(i-1)
        inimesed.pop(i-1)
    elif x=="1":
        i=0
        keda=input("Sisestage nimi: ")
        n=len(inimesed)
        while i<n:
            if keda==inimesed[i]:
                inimesed.pop(i)
